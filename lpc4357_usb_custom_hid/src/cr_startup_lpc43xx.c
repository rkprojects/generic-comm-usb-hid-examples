...
//*****************************************************************************
void ResetISR(void) {
	// Emulate what cortex cpu does post reset:
	// Re-Initialize Stack pointer in case executing from RAM, as
	// the previous flash code might have different stack location.
	__asm volatile ("msr msp, %0" :: "rm" (&_vStackTop):);
	__asm volatile ("msr psp, %0" :: "rm" (&_vStackTop):);
	__asm volatile ("isb");

	ResetISR2(); //adjust all stack/frame pointers.
}
